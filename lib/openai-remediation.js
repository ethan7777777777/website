          ...
          content: [
            {
              ...
              text: [
                ...
                "Real estate sites often have multiple forms (contact, property inquiry, investor intake). Ensure notice-at-collection coverage is applied across all detected forms in a non-breaking additive way.",
                ...
                "Return only additive compliance module markup for remediated_html (scripts/styles/inline snippets only, no full-page replacement).",
                `Source HTML:\n${trimHtml(input.html)}`
              ].join("\n\n")
            }
          ]
...
async function generateRemediationWithModel(input) {
  ...
  let reviewed;
  try {
    reviewed = await reviewCandidateHtml(input, apiKey, generated);
  } catch (error) {
    reviewed = {
      review_model: getReviewModel(),
      approved: true,
      summary: `Generated remediation applied without review validation: ${error.message || "Unknown review failure"}`,
      breakage_risks: [
        "Review model validation failed; apply extra manual QA before publishing to production."
      ],
      changes_applied: ["Applied generation-model compliance module output."],
      required_followups: ["Run a browser QA pass on key pages and forms."],
      remediated_html: generated.candidate_html,
      raw_json: {
        approved: false,
        review_error: error.message || "Unknown review failure",
        used_generation_without_review: true
      }
    };
  }
