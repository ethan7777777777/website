...
async function directWebsiteFallback(url, reason) {
  const targets = uniqueUrls([
    url,
    toAbsoluteUrl(url, "/privacy-policy"),
    toAbsoluteUrl(url, "/privacy"),
    toAbsoluteUrl(url, "/privacy-practices"),
...
async function scrapeWebsiteBundle(url) {
  const apiKey = getFirecrawlKey();
  if (!apiKey) {
    return directWebsiteFallback(url, "missing_firecrawl_key");
  }
  ...
  try {
    ...
    if (!combinedHtml && !combinedMarkdown) {
      return directWebsiteFallback(url, extractError.message);
    }
